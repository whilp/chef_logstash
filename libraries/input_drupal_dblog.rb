...
class Chef
  class Resource
    class LogstashInputDrupal_dblog < Chef::Resource::LogstashConfig

      def tags(arg=nil)
        set_or_return(:tags, arg, :kind_of => [Array])
      end

      def add_usernames(arg=nil)
        set_or_return(:add_usernames, arg, :kind_of => [TrueClass, FalseClass])
      end
      ...
      def databases(arg=nil)
        set_or_return(:databases, arg, :kind_of => [Hash])
      end

      def bulksize(arg=nil)
        set_or_return(:bulksize, arg, :kind_of => [Fixnum])
      end

      def interval(arg=nil)
        set_or_return(:interval, arg, :kind_of => [Fixnum])
      end
